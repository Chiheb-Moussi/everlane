...
import {Categorie, LinkType, SubCategorie} from "../SubMenu/Menu.ts";
import {useState} from "react";
import {twMerge} from "tailwind-merge";
import SubCategories from "../SubMenu/SubCategories.tsx";
...
const TopMenu = () => {
  ...
  return (
      <div className="w-full border-b border-zinc-300 ">
        <div
            className="relative w-full h-14 sm:container  justify-between items-center flex gap-4">
          <div className=" hidden sm:w-fit sm:justify-start sm:items-start sm:flex">

            {menuData.map((category: Categorie) => (
                <button
                    className={twMerge("text-center text-neutral-800 text-xs font-normal font-['Maison Neue'] leading-none tracking-tight cursor-pointer p-3 border-b-2 border-transparent", selectedItem?.name === category.name ? ' border-black' : '')}
                    key={category.name}
          ...
          <div className="absolute top-10">{currentSubCategories.length > 0 && <SubCategories data={currentSubCategories}/>}</div>
